...
/* Domains that must never be intercepted (e.g. external API/webhook calls) */
var PASSTHROUGH_ORIGINS = [
  'https://webhook.site',
  'https://corsproxy.io'
];
...
/* Pass through any explicitly allowlisted external origins untouched */
var reqOrigin = new URL(event.request.url).origin;
for (var i = 0; i < PASSTHROUGH_ORIGINS.length; i++) {
  if (reqOrigin === PASSTHROUGH_ORIGINS[i]) return;
}
